...
aprs_call = "urcall"
aprs_pass = "*****"
aprs_qth = "52xx.15N/013xx.88E"
mqtt_broker = "urbroker_ip"
mqtt_topic = "7ata/wx/"
mqtt_rx = "raw"  	# Einlesen JSON aus rtl_433
mqtt_ch = "dev"  	# Selbst erstellte JSON wird in "mqtt_topic/mqtt_ch" veröffentlicht  dev = develop, tgo = live!
...
def schreiben_JSON(data):
    ...
    client.publish(mqtt_topic + mqtt_ch, str(publ_data))
...
def regen_ermitteln(data):
    ...
    if (int(time.time()) - count_rain_5m) > 300:
        #print(datetime.fromtimestamp(int(time.time())), "  vor 5m.", data['rain_mm'], "rain_mm_1h|24h/Count_rain_1h|24h/mn: ",\
        #      sum(rain_mm_1h), count_rain_1h, "|", sum(rain_mm_24h), count_rain_24h, rain_mn)	#, "\r\b")
        ...
        if mqtt_ch == "dev" and sum(rain_mm_1h) > 20:
            print(datetime.fromtimestamp(int(time.time())), " > 10mm in 1h!! ", data['rain_mm'], "rain_mm_1h|24h/Count_rain_1h|24h/mn: ",\
...
def wind_ermitteln(data):
    ...
    #if mqtt_ch == "dev":
    #    print(datetime.fromtimestamp(int(time.time())), " max:", wind_5m, wind_5m_max, "  \r\b")
    # |count-10m:", count_10m, "|count-5m", count_5m,
    #          "|avg:", wind_10m, "|gust:", wind_5m, "|RAW:", data['wind_avg_m_s'], data['wind_max_m_s'], " "*50,  "\r\b")

    return(wind_10m, wind_5m)

def meldung(data):
    global counter, aprs_remark, aprs_timer, gestern, rain_mn
    ...
    if 'rain_mm' in data:
        dev_vorher = "Vor regen_ernmitteln:" + \
                     str(data_tx['Regen_1h']) + " " + \
                     str(data_tx['Regen_24h']) + " " + \
                     str(data_tx['Regen_mn'])
        ...
        #if mqtt_ch == "dev":
        #    print(datetime.fromtimestamp(int(time.time())), dev_vorher, \
        #          " Nach:", data_tx['Regen_1h'], data_tx['Regen_24h'], \
        #          data_tx['Regen_mn'], " "*45, "\r\b")
    ...
    aprs_login = "user " + aprs_call + " pass " + aprs_pass +\
    "\n" + aprs_call + "-12>APRS,TCPIP*:" + \
    "@" + aprs_zeit + "h" + aprs_qth + "_"
    ...
    if aprs_diff > timing:
        ...
        if mqtt_ch != "dev":
            client.publish(mqtt_topic + "aprs", msg)
        else:

            #print(datetime.fromtimestamp(int(time.time())), msg)
            if 'temperature_C' in data:
                print(datetime.fromtimestamp(int(time.time())), " akt./max./min. Temperatur:",
                      ...
                      "Rain 1h/24H/mn:", data_tx['Regen_1h'], data_tx['Regen_24h'], data_tx['Regen_mn'],
                      "Hpa:", data_tx['Luftdruck'], "   \r\b")

    # Tageswechsel
    if not gestern == heute:
        # Tagesstatistik schreiben
        tag_inhalt = str(datetime.fromtimestamp(int(time.time()))) + " max./min. Temperatur: " + \
                     str(temp_max_tag) + "/" + str(temp_min_tag) + ", max. Windböen: " + str(wind_max_tag) + \
                     ", Regen seit 0h: " + str(round(rain_mn,1)) + "\n"
...
print(datetime.now().strftime("%H:%M:%S"),
      ...
      farbe_aus, "mit rx-ch", farbe_gelb, mqtt_rx, farbe_aus, "und tx-ch",
